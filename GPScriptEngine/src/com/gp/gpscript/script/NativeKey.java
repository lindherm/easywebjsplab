 ...
 * </p>
 * <p>
 * Description: This class implements the Key (a built-in object). See [GP_SYS_SCR] 7.1.11
 * </p>
 * <p>
...
public class NativeKey extends IdScriptableObject {
	...
	public static void init(Context cx, Scriptable scope, boolean sealed) {
		...
		prototypeKey.defineProperty("PERMANENT", ScriptRuntime.wrapNumber(PERMANENT), attr);
		prototypeKey.defineProperty("NONE", ScriptRuntime.wrapNumber(NONE), attr);
		prototypeKey.defineProperty("CRT_PRIVATE", ScriptRuntime.wrapNumber(CRT_PRIVATE), attr);
		prototypeKey.defineProperty("OTHER", ScriptRuntime.wrapNumber(OTHER), attr);
		prototypeKey.defineProperty("SECRET", ScriptRuntime.wrapNumber(SECRET), attr);
		prototypeKey.defineProperty("PRIVATE", ScriptRuntime.wrapNumber(PRIVATE), attr);
		prototypeKey.defineProperty("PUBLIC", ScriptRuntime.wrapNumber(PUBLIC), attr);
		prototypeKey.defineProperty("ENCRYPT", ScriptRuntime.wrapNumber(ENCRYPT), attr);
		prototypeKey.defineProperty("DECRYPT", ScriptRuntime.wrapNumber(DECRYPT), attr);
		prototypeKey.defineProperty("DECRYPT_ENCRYPT", ScriptRuntime.wrapNumber(DECRYPT_ENCRYPT), attr);
		prototypeKey.defineProperty("DERIVE", ScriptRuntime.wrapNumber(DERIVE), attr);
		prototypeKey.defineProperty("GENERATE_KEY", ScriptRuntime.wrapNumber(GENERATE_KEY), attr);
		prototypeKey.defineProperty("SIGN", ScriptRuntime.wrapNumber(SIGN), attr);
		prototypeKey.defineProperty("VERIFY", ScriptRuntime.wrapNumber(VERIFY), attr);
		prototypeKey.defineProperty("WRAP", ScriptRuntime.wrapNumber(WRAP), attr);
		prototypeKey.defineProperty("UNWRAP", ScriptRuntime.wrapNumber(UNWRAP), attr);
		prototypeKey.defineProperty("UNWRAP_WRAP", ScriptRuntime.wrapNumber(UNWRAP_WRAP), attr);
		prototypeKey.defineProperty("CRT_P", ScriptRuntime.wrapNumber(CRT_P), attr);
		prototypeKey.defineProperty("CRT_Q", ScriptRuntime.wrapNumber(CRT_Q), attr);
		prototypeKey.defineProperty("CRT_DP1", ScriptRuntime.wrapNumber(CRT_DP1), attr);
		prototypeKey.defineProperty("CRT_DQ1", ScriptRuntime.wrapNumber(CRT_DQ1), attr);
		prototypeKey.defineProperty("CRT_PQ", ScriptRuntime.wrapNumber(CRT_PQ), attr);
		prototypeKey.defineProperty("EXPONENT", ScriptRuntime.wrapNumber(EXPONENT), attr);
		prototypeKey.defineProperty("MODULUS", ScriptRuntime.wrapNumber(MODULUS), attr);
		prototypeKey.defineProperty("DES", ScriptRuntime.wrapNumber(DES), attr);
		if (sealed) {
			prototypeKey.sealObject();
		}
		ScriptableObject.defineProperty(scope, "Key", prototypeKey, ScriptableObject.DONTENUM);
	}
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		switch (id) {
		/*
		 * case ConstructorId_fromCharCode: return jsStaticFunction_fromCharCode(args); case Id_constructor: return jsConstructor(args, thisObj == null);
		 */case Id_getAttribute:
			return realThis(thisObj, f).jsFunction_getAttribute(cx, scope, args);
		case Id_getBlob:
			return realThis(thisObj, f).jsFunction_getBlob(cx, scope, args);
		case Id_getBlobFormat:
			return realThis(thisObj, f).jsFunction_getBlobFormat(cx, scope, args);
		case Id_getEnd:
			return realThis(thisObj, f).jsFunction_getEnd(cx, scope, args);
		case Id_getComponent:
			return realThis(thisObj, f).jsFunction_getComponent(cx, scope, args);
		case Id_getKcv:
			return realThis(thisObj, f).jsFunction_getKcv(cx, scope, args);
		case Id_getOwner:
			return realThis(thisObj, f).jsFunction_getOwner(cx, scope, args);
		case Id_getProfileID:
			return realThis(thisObj, f).jsFunction_getProfileID(cx, scope, args);
		case Id_getSize:
			return realThis(thisObj, f).jsFunction_getSize(cx, scope, args);
		case Id_getStart:
			return realThis(thisObj, f).jsFunction_getStart(cx, scope, args);
		case Id_getType:
			return realThis(thisObj, f).jsFunction_getType(cx, scope, args);
		case Id_getUsage:
			return realThis(thisObj, f).jsFunction_getUsage(cx, scope, args);
		case Id_getVersion:
			return realThis(thisObj, f).jsFunction_getVersion(cx, scope, args);
		case Id_getWrapKey:
	...
	/*
	 * private static Object jsConstructor(Object[] args, boolean inNewExpr) { if(args.length==1) //name return new NativeKey((String)args[0]); if(args.length==2) //name+blob return new NativeKey((String)args[0],(String)args[1]); if(args.length==3) //name+blob+keyProfile return new NativeKey((String)args[0],(String)args[1],(KeyProfile)args[2]); return new NativeKey(); }
	 * 
	 * //for return a new ByteString from a ByteString Object public static NativeKey newKey(Context cx, Scriptable scope,NativeKey sNew) { scope = getTopLevelScope(scope); Scriptable result = ScriptRuntime.newObject(cx, scope, "Key", null); NativeKey NewKey=(NativeKey)result; NewKey.strIndex=sNew.strIndex; return NewKey; }
	 */
	...
	protected int findPrototypeId(String s) {
		...
		if (s.equals("getStart")) {
			X = "getStart";
			id = Id_getStart;
		}
		if (s.equals("getUsage")) {
			X = "getUsage";
			id = Id_getUsage;
		}
		if (s.equals("setOwner")) {
			X = "setOwner";
			id = Id_setOwner;
		}
		if (s.equals("setStart")) {
			X = "setStart";
			id = Id_setStart;
		}
		if (s.equals("setUsage")) {
			X = "setUsage";
			id = Id_setUsage;
		}
		...
		if (s.equals("getWrapKey")) {
			X = "getWrapKey";
			id = Id_getWrapKey;
		}
		if (s.equals("setVersion")) {
			X = "setVersion";
			id = Id_setVersion;
		}
		if (s.equals("setWrapKey")) {
			X = "setWrapKey";
			id = Id_setWrapKey;
		}
		...
		if (s.equals("getComponent")) {
			X = "getComponent";
			id = Id_getComponent;
		}
		if (s.equals("getProfileID")) {
			X = "getProfileID";
			id = Id_getProfileID;
		}
		...
		if (s.equals("setComponent")) {
			X = "setComponent";
			id = Id_setComponent;
		}
		if (s.equals("getBlobFormat")) {
			X = "getBlobFormat";
			id = Id_getBlobFormat;
		}
		if (s.equals("setBlobFormat")) {
			X = "setBlobFormat";
			id = Id_setBlobFormat;
		}
	...
	// Id_constructor = 1,
			Id_getAttribute = 1,
			Id_getBlob = 2, Id_getBlobFormat = 3, Id_getEnd = 4, Id_getComponent = 5, Id_getKcv = 6, Id_getOwner = 7, Id_getProfileID = 8, Id_getSize = 9, Id_getStart = 10, Id_getType = 11, Id_getUsage = 12, Id_getVersion = 13, Id_getWrapKey = 14, Id_setAttribute = 15, Id_setBlobFormat = 16, Id_setEnd = 17, Id_setComponent = 18, Id_setOwner = 19, Id_setSize = 20, Id_setStart = 21, Id_setType = 22, Id_setUsage = 23, Id_setVersion = 24, Id_setWrapKey = 25, MAX_PROTOTYPE_ID = 25;
	...
	public static final int PERMANENT = 0x01, NONE = 0x01, CRT_PRIVATE = 0x02, OTHER = 0x04, SECRET = 0x01, PRIVATE = 0x02, PUBLIC = 0x04, ENCRYPT = 0x01, DECRYPT = 0x02, DECRYPT_ENCRYPT = 0x04, DERIVE = 0x08, GENERATE_KEY = 0x10, SIGN = 0x20, VERIFY = 0x40, WRAP = 0x80, UNWRAP = 0x100, UNWRAP_WRAP = 0x200, CRT_P = 0x01, CRT_Q = 0x02, CRT_DP1 = 0x04, CRT_DQ1 = 0x08, CRT_PQ = 0x10, EXPONENT = 0x20, MODULUS = 0x40, DES = 0x80;

	/*
	 * public static void main(String[] args) { System.out.println(NativeKey.getKcv("404142434445464748494A4B4C4D4E4F")); }
	 */
}
