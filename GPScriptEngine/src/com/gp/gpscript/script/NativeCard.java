 ...
 * <p>Title: Card</p>
 * <p>Description:
 * This class implements the Card (a biult-in object).
 * See [GP_SYS_SCR] 7.1.10 </p>
...
public class NativeCard extends IdScriptableObject
{
    ...
    public static void init(Context cx, Scriptable scope, boolean sealed)
    {
        ...
        if (sealed) { prototypeCard.sealObject(); }
        ScriptableObject.defineProperty(scope, "Card", prototypeCard,
    ...
    public String getClassName() { return "Card"; }
...
//        setParentScope(prototypeCard.getParentScope());
    ...
    {
        if (id == Id_response) { return "response"; }
        if (id == Id_profile) { return "profile"; }
        if (id == Id_SW) { return "SW"; }
        if (id == Id_SW1) { return "SW1"; }
        if (id == Id_SW2) { return "SW2"; }
 ...
 */
   ...
   /**
    * and throw exception according to the expected sw and the result sw
    * this function is used by sendApdu
    * @param strResp
    * @param sw
    */
     ...
     * this function is used by sendApdu
     * @param cx
     * @param className the class which send Apdu
     * @param strResp the response data of the command
     * @param sw  SW1SW2
     */
    public static void SWException(Context cx, String className, String strResp,
                                   int sw) {
        String strSW;
        strSW = strResp.substring(strResp.length() - 2 * 2); //card sw
        if (sw == 0x9000) {
            if (!strSW.equals("9000"))
                throw new EvaluatorException( ( new GPError(className, 0, 0,
                    "Parameter sw[] not present and card SW<>0x9000---SW=" +
                                  strSW)).toString());
        }
        else {
            //exception
    ...
    public byte[] sendApdu(Context cx, Scriptable scope, byte[] toSendData,  int len)
    {
     ...
     public byte[] sendApdu(Context cx, Scriptable scope,
                            int CLA,
                            int INS,
                            int P1,
                            int P2,
     ...
     * @param p3 PP1
     * @param p4 PP2
     * @param p5 commdata (default=null)
     * @param p6 LE(default is -1)
     * @param p7 SW(default is 0x9000)
   ...
   public NativeByteString sendApdu(Context cx, Scriptable scope,Number p1,Number p2,Number p3,Number p4,NativeByteString p5,Number p6,Number p7)
   {
      ...
      if(p5 instanceof NativeByteString)
      {
          int LC=p5.GetLength();
          data=new byte[LC];
          for(int i=0;i<LC;i++) data[i]=p5.ByteAt(i);
      }
      //send command
      byte[] resp=new byte[500];
...
//**************************************************/
    ...
    protected void initPrototypeId(int id)
    {
        if (id <= LAST_METHOD_ID) {
            ...
            switch (id) {
            //function
            case Id_sendApdu: arity = 7; name = "sendApdu"; break;
            case Id_reset:  arity = 0; name = "reset"; break;
...
/*        else {
            ...
            //constant
            switch (id) {
              case Id_T0:  x = T0; name = "T0"; break;
              case Id_T1:  x = T1; name = "T1"; break;
              case Id_T14: x = T14; name = "T14"; break;
              case Id_OTHER: x = OTHER; name = "OTHER"; break;
              case Id_RESET_WARM: x = RESET_WARM; name = "RESET_WARM"; break;
              case Id_RESET_COLD: x = RESET_COLD; name = "RESET_COLD"; break;
              default: throw new IllegalStateException(String.valueOf(id));
            }
...
/*        if(s.equals("T0")) {X="T0";id=Id_T0;}
        if(s.equals("T1")) {X="T1";id=Id_T1;}
        if(s.equals("SW")) {X="SW";id=Id_SW;}
        if(s.equals("SW1")) {X="SW1";id=Id_SW1;}
        if(s.equals("SW2")) {X="SW2";id=Id_SW2;}
        if(s.equals("T14")) {X="T14";id=Id_T14;}
        if(s.equals("OTHER")) {X="OTHER";id=Id_OTHER;}
        if(s.equals("profile")) {X="profile";id=Id_profile;}
        if(s.equals("RESET_WARM")) {X="RESET_WARM";id=Id_RESET_WARM;}
        if(s.equals("RESET_COLD")) {X="RESET_COLD";id=Id_RESET_COLD;}
        if(s.equals("response")) {X="response";id=Id_response;}
*/
        if(s.equals("reset")) {X="reset";id=Id_reset;}
...
/*
    public ApduChannel getApduChannel()
    {
      return apduChannel;
    }
*/
	...
	Id_T0 	=	 LAST_METHOD_ID+1,
   	Id_T1 	=	 LAST_METHOD_ID+2,
   	Id_T14     	=	 LAST_METHOD_ID+3,
 	Id_OTHER   	=	 LAST_METHOD_ID+4,
  	Id_RESET_WARM     	=	 LAST_METHOD_ID+5,
   	Id_RESET_COLD      = LAST_METHOD_ID+6,
        //property
   	Id_SW      = LAST_METHOD_ID+7,
   	Id_SW1      = LAST_METHOD_ID+8,
   	Id_SW2      = LAST_METHOD_ID+9,
   	Id_response      = LAST_METHOD_ID+10,
        Id_profile      = LAST_METHOD_ID+11,
