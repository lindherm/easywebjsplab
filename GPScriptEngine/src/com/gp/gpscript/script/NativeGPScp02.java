 ...
 * </p>
 * <p>
 * Description: This class implements the GPScp02 (a built-in object). See [GP_SYS_SCR] 7.1.9
 * </p>
 * <p>
...
public class NativeGPScp02 extends IdScriptableObject {
	...
	protected String getInstanceIdName(int id) {
		/*
		 * if (id == Id_card ) { return "card"; } if (id == Id_data ) { return "data"; } if (id == Id_key ) { return "key"; } if (id == Id_profile ) { return "profile"; }
		 */
		if (id == Id_crypto) {
			return "crypto";
		}
	...
	private NativeByteString initializeUpdate(Context cx, Scriptable scope, Number p1, Number p2) {
		...
		/*
		 * try { hostRandom = Crypto.generaterandom(8); } catch (Exception e) { e.printStackTrace(); throw new EvaluatorException( ( new GPError("GPScp02", 0, 0, e.getMessage())).toString()); }
		 */
		...
		/*
		 * GP_Global.setValue("AID",strResp.substring(0,4)); GP_Global.setValue("CSN",strResp.substring(8,16)); if(GP_Global.appStyle=="APP") { ScriptRuntime.setElem(NativeApplication.data,"AID", new NativeByteString(strResp.substring(0,4),new Integer(HEX)),scope); ScriptRuntime.setElem(NativeApplication.data,"CSN", new NativeByteString(strResp.substring(8,16),new Integer(HEX)),scope); } else if(GP_Global.appStyle=="GPAPP") { ScriptRuntime.setElem(NativeGPApplication.data,"AID", new NativeByteString(strResp.substring(0,4),new Integer(HEX)),scope); ScriptRuntime.setElem(NativeGPApplication.data,"CSN", new NativeByteString(strResp.substring(8,16),new Integer(HEX)),scope); } else//GPSEC { ScriptRuntime.setElem(NativeGPSecurityDomain.data,"AID", new NativeByteString(strResp.substring(0,4),new Integer(HEX)),scope); ScriptRuntime.setElem(NativeGPSecurityDomain.data,"CSN", new NativeByteString(strResp.substring(8,16),new Integer(HEX)),scope); }
		 */
	...
	/**
	 * 适应江苏市民卡项目,randomData为传入随机数 使用传入的随机数进行initializeUpdate
	 * 
	 * @param cx
	 * @param scope
	 * @param p1
	...
	private NativeByteString decryptEncryptKek(Context cx, Scriptable scope, NativeKey p1, Number p2, NativeByteString p3, NativeByteString p4) {
		...
		/*
		 * try { dataToEncrypt=Crypto.decrypt(decryptKey,decryptMech,dataToDecrypt,decryptIv); } catch (CryptoException e) { e.printStackTrace(); throw new EvaluatorException( ( new GPError("GPScp02", 0, 0, e.getMessage())).toString()); }
		 */
		// encrypt use KEK
		 ...
		 * try {
		 * 
		 * out=Crypto.encrypt(wrapKey,encryptMech,dataToEncrypt,encryptIV); } catch (CryptoException e) { e.printStackTrace(); System.out.println(e.getMessage()); }
		 */
		// return
	...
	public NativeByteString encryptKek(Context cx, Scriptable scope, NativeByteString p1) {
		 ...
		 * try {
		 * 
		 * out=Crypto.encrypt(wrapKey,mech,dataToEncrypt,iv); } catch (CryptoException e) { e.printStackTrace(); System.out.println(e.getMessage()); }
		 */
		// return
		String str = new String(Hex.encode(out));
		Integer ee = new Integer(GPConstant.HEX);
	...
	private void jsFunction_unwrapWrapKek(Context cx, Scriptable scope, Object[] args) {
		 ...
		 * try {
		 * 
		 * out=Crypto.decrypt(unwrapKey,unwrapMech,keyToUnwrap,unwrapIV); } catch (CryptoException e) { e.printStackTrace(); throw new EvaluatorException( ( new GPError("GPScp02", 0, 0, e.getMessage())).toString()); }
		 */
		// encrypt
		byte[] out1 = null;
		 ...
		 * try {
		 * 
		 * out1=Crypto.encrypt(wrapKey,wrapMech,out,wrapIV); } catch (CryptoException e) { e.printStackTrace(); throw new EvaluatorException( ( new GPError("GPScp02", 0, 0, e.getMessage())).toString()); }
		 */
		// update database
	...
	protected void initPrototypeId(int id) {
		...
		case Id_initializeUpdateV2:
			arity = 2;
			s = "initializeUpdateV2";
			break;
		case Id_externalAuthenticateV2:
			arity = 1;
			s = "externalAuthenticateV2";
			break;
		case Id_externalAuthenticateLmk:
			arity = 1;
			s = "externalAuthenticateLmk";
			break;
		case Id_decryptEncryptKek:
			arity = 4;
			s = "decryptEncryptKek";
		...
		case Id_endRMac:
			...
			break;
		/*
		 * //property case Id_card: return "card"; case Id_data: return "data"; case Id_key: return "key"; case Id_profile: return "profile"; case Id_crypto: return "crypto"; case Id_cardChallenge: return "cardChallenge"; case Id_cardCryptogram: return "cardCryptogram"; case Id_diversificationData: return "diversificationData"; case Id_hostChallenge: return "hostChallenge"; case Id_keyIndex: return "keyIndex"; case Id_keyVersion: return "keyVersion"; case Id_smac: return "smac"; case Id_rmac: return "rmac"; case Id_option: return "option"; case Id_sequenceCounter: return "sequenceCounter"; case Id_securityLevel: return "securityLevel"; case Id_state: return "state";
		 */
		default:
			throw new IllegalArgumentException(String.valueOf(id));
		}
	...
	protected int findPrototypeId(String s) {
		...
		String X = null;
		/*
		 * if(s.equals("key")) {X="key";id=Id_key;} if(s.equals("card")) {X="card";id=Id_card;} if(s.equals("data")) {X="data";id=Id_data;} if(s.equals("profile")) {X="profile";id=Id_profile;}
		 */
		if (s.equals("smac")) {
			X = "smac";
			id = Id_smac;
		...
		if (s.equals("cardCryptogram")) {
			X = "cardCryptogram";
			id = Id_cardCryptogram;
		}
		if (s.equals("sequenceCounter")) {
			X = "sequenceCounter";
			id = Id_sequenceCounter;
		}
		...
		if (s.equals("diversificationData")) {
			X = "diversificationData";
			id = Id_diversificationData;
		}
		if (s.equals("externalAuthenticate")) {
			X = "externalAuthenticate";
			id = Id_externalAuthenticate;
		}
		if (s.equals("initializeUpdateV2")) {
			X = "initializeUpdateV2";
			id = Id_initializeUpdateV2;
		}
		if (s.equals("externalAuthenticateV2")) {
			X = "externalAuthenticateV2";
			id = Id_externalAuthenticateV2;
		}
		if (s.equals("externalAuthenticateLmk")) {
			X = "externalAuthenticateLmk";
			id = Id_externalAuthenticateLmk;
		}
		return id;
	}

	// define value for all function and constant
	private static final int Id_decryptEncryptKek = 1, Id_encryptKek = 2, Id_externalAuthenticate = 3, Id_getKekKey = 4, Id_initializeUpdate = 5, Id_setEncKey = 6, Id_setKekKey = 7, Id_setMacKey = 8, Id_unwrapWrapKek = 9, Id_setBaseKey = 10, Id_beginRMac = 11, Id_endRMac = 12, Id_initializeUpdateV2 = 13, Id_externalAuthenticateV2 = 14, Id_externalAuthenticateLmk = 15, LAST_METHOD_ID = 15, MAX_PROTOTYPE_ID = 15;
	// the following should be contants

	private static final int Id_cardChallenge = 1, Id_cardCryptogram = 2, Id_crypto = 3, Id_diversificationData = 4, Id_hostChallenge = 5, Id_keyIndex = 6, Id_keyVersion = 7, Id_smac = 8, Id_securityLevel = 9, Id_state = 10, Id_option = 11, Id_rmac = 12, Id_sequenceCounter = 13, MAX_INSTANCE_ID = 13;
