 ...
 * </p>
 * <p>
 * Description: This class implements the GPApplication (a built-in object). See [GP_SYS_SCR] 7.1.5
 * </p>
 * <p>
...
public class NativeGPApplication extends IdScriptableObject {
	...
	public static void init(Context cx, Scriptable scope, boolean sealed) {
		...
		if (sealed) {
			prototypeGPApplication.sealObject();
		}
	}
	...
	/**
	 * ByteString getData(Number tag) ByteString getData(Number tag, Number sw)
	 * 
	 * @param cx
	 *            context
	 * @param scope
	...
	/**
	 * ByteString getStatus(Number type,ByteString criteria,Boolean next) ByteString getStatus(Number type,ByteString criteria,Boolean next,Number sw)
	 * 
	 * @param cx
	 * @param scope
	 * @param args
	...
	private NativeByteString jsFunction_getStatus(Context cx, Scriptable scope, Object[] args) {
		...
		switch (type) {
		case GPConstant.CM_ONLY:
			P1 = new Integer(0x80);
			break;
		case GPConstant.APPS_ONLY:
			P1 = new Integer(0x40);
			break;
		case GPConstant.LF_ONLY:
			P1 = new Integer(0x20);
			break;
		case GPConstant.LFE_ONLY:
			P1 = new Integer(0x10);
			break;
		}
		if (((Boolean) args[2]).booleanValue() == true)
			P2 = new Integer(0x01);
	 ...
	 * @param scope
	 * @param args
	 *            args should be more and sw more default is false sw default is 0x9000 send APDU(0x80 0xD8 .....) should encrypt key with the card KEK
	 * @return
	 */
	...
	private NativeByteString jsFunction_putKey(Context cx, Scriptable scope, Object[] args) {
		...
		for (int i = 1; i < (args.length / 3); i++) {
			...
			/*
			 * NativeKey KEK=(NativeKey)key.get("Sec_KEK",scope);//new NativeKey("KDCkek"); NativeByteString iv=new NativeByteString("0000000000000000",new Integer(GPConstant.HEX)); NativeByteString encryptedKey =NativeCrypto.encrypt(KEK,new Integer(Crypto.DES_ECB),key,iv);
			 */
	 ...
	 * @param scope
	 * @param args
	 *            args should be status and sw sw default is 0x9000 send APDU(80 F0 80 status LC AID) AID should be retrieved from the card profile
	 * @return no return
	 */
	 ...
	 * @param scope
	 * @param args
	 *            args should be status and sw sw default is 0x9000 send APDU(00 E2 P1 P2 LC AID) ???CLA is 0x00, not 0x80 P1=0x80(last block) P1=0x00(more block) AID should be retrieved from the card profile
	 * @return no return
	 */
	 ...
	 */
	/*
	 * private Number jsFunction_openSecureChannel(Context cx, Scriptable scope,Object[] args) { Object result = null; int level=((Number)args[0]).intValue(); String strLevel=((Number)args[0]).toString(); cx.evaluateString(scope, "level="+strLevel+";" + this.profile.SecureChannel.OpenSecureChannel.Script.Script, "OpenSecureChannel", 0, null); secureLevel=level; return new Integer(level);
	 */
	/*
	 * String fileName=GP_Global.scriptDir+GP_Global.openSecureChannelScriptFile; //create the scripts try { String secureChannelScript=""; secureChannelScript=secureChannelScript+"level="+strLevel+";"; secureChannelScript=secureChannelScript+this.profile.SecureChannel.OpenSecureChannel.Script.Script; DataOutputStream out=new DataOutputStream(new FileOutputStream(fileName)); out.writeBytes(secureChannelScript); out.close(); } catch(Exception e) { e.printStackTrace(); System.out.println(e.getMessage()); } //execute the script try { result = RunScript.processSource(cx, scope, fileName); System.out.println("openSecureChannel is successed!"); GpSecureLevel=level; //GPApp will impact the command of the GPSecurityDomain securityDomain.GpSecDomainSecureLevel=level; return new Integer(level); } catch (GPError egErr) { egErr.printStackTrace(); System.out.println(egErr.toString()); // Exit from the context. Context.exit(); return new Integer(-1); } }
	 */
	...
	protected void initPrototypeId(int id) {
		...
		/*
		 * //constant case Id_card: return "card"; case Id_crypto: return "crypto"; case Id_data: return "data"; case Id_key: return "key"; case Id_profile: return "profile"; case Id_securityDomain: return "securityDomain"; case Id_secureChannel: return "secureChannel";
		 */
		default:
			throw new IllegalArgumentException(String.valueOf(id));
		}
	...
	private static final int Id_getData = 1, Id_getStatus = 2, Id_putKey = 3, Id_select = 4, Id_sendApdu = 5, Id_setStatus = 6, Id_storeData = 7,
	// Id_openSecureChannel=8,
			LAST_METHOD_ID = 7, MAX_PROTOTYPE_ID = 7;
	// the following should be contants
	private static final int Id_card = 1, Id_crypto = 2, Id_data = 3, Id_key = 4, Id_profile = 5, Id_securityDomain = 6,
	// Id_secureChannel = 7,
			MAX_INSTANCE_ID = 6;
