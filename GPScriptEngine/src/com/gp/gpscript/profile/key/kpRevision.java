...
public class kpRevision extends ProfileNode {
	...
	/**
	 * Version of Key Profile conforming to GP versioning specification. Version number is x.x.x where each x is a decimal number. If the version of the specification has no last x value, then version number is x.x.0. Example: 1.0.0
	 */
	...
	public kpRevision(Node node) {
		...
		try {
			...
			nl = xPathNode.getNodeList(xpString, node);
			if (nl.getLength() > 0)
				Signature = new kpSignature(nl.item(0));
		} catch (Exception e) {
			// e.printStackTrace();
