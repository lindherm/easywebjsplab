...
ApplicationProfile type attribute:
ApplicationProfile.ApplicationInfo.Type
Card Profile RAM available attribute:
CardProfile.CardInfo.ResourcesAvailable.
RAM
Key Profile key type attribute:
KeyProfile.KeyInfo.Type
Load File Profile module identifer:
LoadFileProfile.Module.ModuleID
*/
...
Target should contain this value. For
example: 1000
The Target attribute always begins with the
parent element of the profile
(ApplicationProfile, CardProfile, KeyProfile, or
LoadFileProfile). However, since Target can
also contain a constant value for comparison,
the following must be reserved words so that
no fixed values can start with:
ApplicationProfile CardProfile KeyProfile LoadFileProfile
For a target specifying a profile, the format
corresponds to the profile being specified and
the location of the attribute (or element) within
the XML document tree. For example:
ApplicationProfile type attribute:
ApplicationProfile.ApplicationInfo.Type
Card Profile RAM available attribute:
CardProfile.CardInfo.ResourcesAvailable.
...
Key Profile key type attribute:
KeyProfile.KeyInfo.Type
Load File Profile module identifer attribute:
LoadFileProfile.Module.ModuleID
*/
