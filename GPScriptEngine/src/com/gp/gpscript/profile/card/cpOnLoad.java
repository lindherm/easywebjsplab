...
/**
A placeholder element for application provided data used by the SCMS to manage an unique card holder��s smart
card. If more than one Execute element is present, it is up to the discretion of the SCMS to choose the correct
one. Also, there is a capability to specify a simple script, devoid of external data, parameter or key references,
under the SimpleScript element.
*/
...
public		cpExecute	Execute[];
	...
	{	super(node);
		...
		try{
				...
				if(nl.getLength()>0)SimpleScript=new cpSimpleScript(nl.item(0));
				}catch(Exception e)
				{
//                    e.printStackTrace();
