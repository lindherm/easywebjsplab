		...
		mouseListener = new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e)) {
					...
					int unit = fontMetrics.getHeight();
					// 当前行数
					int lineNum = e.getY() / unit;
					Element currentElement = root.getElement(lineNum);
					if (currentElement == null) {
						JOptionPane.showMessageDialog(null, "不是编辑区！");
						return;
					}
					// 选中行号所在行
					editor.setSelectionStart(currentElement.getStartOffset());
					editor.setSelectionEnd(currentElement.getEndOffset());
