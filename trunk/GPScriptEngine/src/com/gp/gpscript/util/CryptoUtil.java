...
public class CryptoUtil {
    ...
    public String GPexternalAuthenticate() {
        ...
        byte[] iv = {
            0, 0, 0, 0, 0, 0, 0, 0};
        //Kdcenc
        data = initRes.substring(0, 4) + initRes.substring(8, 16) + "f001"
            + initRes.substring(0, 4) + initRes.substring(8, 16) + "0f01";
        try {
        ...
        //Kdcmac
        data = initRes.substring(0, 4) + initRes.substring(8, 16) + "f002"
            + initRes.substring(0, 4) + initRes.substring(8, 16) + "0f02";
        ...
        //Kdckek
        data = initRes.substring(0, 4) + initRes.substring(8, 16) + "f003"
            + initRes.substring(0, 4) + initRes.substring(8, 16) + "0f03";
        try {
        ...
        data = Rcard.substring(8, 16) + Rter.substring(0, 8)
            + Rcard.substring(0, 8) + Rter.substring(8, 16);
        log.debug("data=" + data);
        //Kscenc
        ...
        //Apdu
        return "8482000010" + new String(Hex.encode(Host))
            + new String(Hex.encode(Smac));
    }
     ...
     * @param digestMech SHA_1 or MD5
     * @param data  Data to operate on
     * @return  A byte array containing the digest of data
     * @throws CryptoException
     */
    public static byte[] digest(int digestMech, byte[] data) throws
        CryptoException {
        Digest digest = null;
        byte[] resBuf = null;
     ...
     * @param rdmLength Length of random data to generate in bytes.
    ...
    public static byte[] generaterandom(int rdmLength) throws CryptoException {
        ...
        try {
            ...
            /*
             * This following call to setSeed() makes the initialisation of the SecureRandom object
...
//            e.printStackTrace();
     ...
     * @param mech  DES_KEY_GEN DES2_KEY_GEN
    ...
    public static KeyParameter generatorKey(int mech) throws CryptoException {
        ...
        /*
         * initialise the key generator with the parameters
         * and finally, generate the key
         */
        switch (mech) {
            case DES_KEY_GEN:
                kg.init(kgp);
    ...
    /**
     * Performs an encryption operarion,using the key specified by the value parameter,
     * the algorithm specified by the mech parameter.
     * @param key  The key to use in encrypt operation
     * @param mech  DEC_ECB or DES_CBC
     * @param dataToEncrypt  Data to encrypt
         * @param iv Intial vector to use if provided and depending if required by the
    ...
    public static byte[] encrypt(byte[] key,
                                 int mech, byte[] dataToEncrypt, byte[] iv) throws
        CryptoException {
    ...
    /*
     * Performs an decryption operarion,using the key specified by the value parameter,
     * the algorithm specified by the mech parameter.
    ...
    public static byte[] decrypt(
        byte[] key, int mech, byte[] dataToDecrypt,
    ...
    /**
     * Performs a decryption operarion using the key specified by the decryptkey parameter,
     * the algorithm specified by the decryptMech parameter, followed immediately by an encrypt
     * operation using the key passed as encryptKey,the mechanism passed as encryptMech .
     * @param decryptkey The key to use in decrypt operation.
     * @param decryptMech DES_ECB or DES_CBC
     * @param encryptkey  The key to use in encrypt operation
     * @param encryptMech DES_ECB or DES_CBC
     * @param data    Data to encrypt
     * @param decryptIV  Intial vector to use for the decrypt operation if provided and depending
     *  if required by the algorthm specified by the mech parameter.
     * @param encryptIV
     * @return A new byte array containing the result of the decrypt operations on data.
     * @throws CryptoException
     */
    public static byte[] decryptEncrypt(
        byte[] decryptkey, int decryptMech,
        byte[] encryptkey, int encryptMech,
        byte[] data, byte[] decryptIV,
    ...
    /**
     * Creates a signature of the data specified,using the key specified by the singingKey parameter,
     * and the algorithm specified by the signingMech parameter.
     * @param signingkey  Key to use for signing operation
     * @param signingMech DES_MAC
     * @param data Data to sign.
     * @return A byte array containing the signature of data
     * @throws CryptoException
     */
    public static byte[] DESsign(
        byte[] signingkey, int signingMech,
     ...
     * Creates a signature of the data specified,using the key specified by the singingKey parameter,
     * and the algorithm specified by the signingMech parameter.
     * @param privParameters The RSA private key to use for signing operation
     * @param data Data to sign.
     * @return  A byte array containing the signature of data
     * @throws CryptoException
     */
     ...
     * Creates a signature of the data specified,using the key specified by the singingKey parameter,
     * and the algorithm specified by the signingMech parameter.
     * @param verifykey The key used for signature verification
     * @param verifyMech DES_MAC or DES_MAC_EMV
     * @param data  Data on which the signature is based (or genreated).
     * @param signature  Signature to verify
     * @return  True for false depending if verify was successful or not
     * @throws CryptoException
     */
    public static boolean DESverify(
        byte[] verifykey,
        int verifyMech,
        byte[] data,
     ...
     * Creates a signature of the data specified,using the key specified by the singingKey parameter,
     * and the algorithm specified by the signingMech parameter.
     * @param verifykey The RSA public key used for signature verification
     * @param data  Data on which the signature is based (or genreated).
     * @param signature  Signature to verify
     * @return  True for false depending if verify was successful or not
     * @throws CryptoException
     */
    public static boolean RSAverify(
     ...
     * @param publicExponent public key Exponent
     * @param strength RSA modulus bit number
     * @return RSA private/public key pair
     * @throws CryptoException
     */
    public static AsymmetricCipherKeyPair generateRSAKeyPair(
        int publicExponent,
        int strength) throws CryptoException {
        ...
        genParam = new RSAKeyGenerationParameters(
            BigInteger.valueOf(publicExponent), new SecureRandom(), strength
            , 25);
    ...
    /**
         * Derives the key derivedKey using the masterKey specified,and the algorithm
     * specified by the mech parameter. The data parameter is used to specify the deversification data
     * to be used in the operation.
     * @param masterKey  The key to derive from
     * @param mech  DES_ECB or DES_CBC
     * @param data  Deriviation data to use
     * @return  Key to derive.
     * @throws CryptoException
     */
    public static byte[] deriveKey(byte[] masterKey, int mech, byte[] data) throws
        CryptoException {
    ...
    public static String singleDesFinalTripeDesMac(String key,
                                            String data,
    ...
    public static String fullTripeDesMac(String key,
                                         String data,
                                         byte[] icv) throws Exception{
    ...
    public static String getHexLen(String str,
                                   int lenSize){
    ...
    public static String getKcv(String key){
        ...
        String kcv = "";
        try {
            kcv = RsaUtil.byte2hex(encrypt(RsaUtil.hex2byte(key),
                                           DES_ECB,
                                           iv,
                                           iv));
...
//            e.printStackTrace();
    ...
    public static String singleDESEncrypt(String key, String data)
            throws Exception {
        byte[] iv = {
                0, 0, 0, 0, 0, 0, 0, 0};
