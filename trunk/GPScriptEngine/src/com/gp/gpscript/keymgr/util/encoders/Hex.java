...
public class Hex
{
    ...
    /**
     * conform hexString to Int
     * for example, "0100"->256
     * @param str
     * @return
     */
    public static int hexStrToInt(String str) {
        ...
        for (int i = 0; i < buf.length; i++) {
            value = value +
                ByteToUnsigned(buf[i]) *
                (int) (Math.pow( (double) 256, (double) (buf.length - i - 1)));
        }
        return (int) value;
    }
    ...
    /**
     * byteToString conform a byte to String(1bytes)
     * data a byte data
     */
