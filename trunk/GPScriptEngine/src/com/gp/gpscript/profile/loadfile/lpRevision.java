...
/**
Version of Load File Profile conforming to GP
	...
	{	super(node);
		...
		if(attr!=null)Digest=attr.getNodeValue();
			...
			try{
				...
				if(nl.getLength()>0)Description=new lpDescription(nl.item(0));
				}catch(Exception e)
				{
//                    e.printStackTrace();
			...
			try{
					xpString="Signature";
					nl=xPathNode.getNodeList(xpString,node);
				if(nl.getLength()>0)Signature=new lpSignature(nl.item(0));
				}catch(Exception e)
				{
//                    e.printStackTrace();
