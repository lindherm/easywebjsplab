...
import com.gp.gpscript.profile.app.apConflictRule;
import com.gp.gpscript.profile.card.CardProfile;
import com.gp.gpscript.profile.card.cpConflictRule;
...
public class ConflictCheck {
	...
	public boolean Check(CardProfile cp, ApplicationProfile ap[]) {
		/*
		 * Conflict Rule Check Flow 1.Check rules associated with Card Profile 1.1.check rules with source type of ApplicationProfile 1.1.1 check rules with target type of Cardprofile 1.1.2 regardless rules with target type of ApplicationProfile/KeyProfile /LoadFileProfile 1.1.3 check rules with target type of Constant value 1.2 check rules with source type of CardProfile 1.2.1 check rules with target type of ApplicationProfile 1.2.2 regardless rules with target type of CardProfile/KeyProfile/LoadFileProfile 1.2.3 check rule of target of constant value 2.check rules associated with Application Profile 2.1 check rules with soure type of ApplicationProfile 2.1.1 check rules with target type of Cardprofile 2.1.2 regardless rules with target type of ApplicationProfile/KeyProfile /LoadFileProfile 2.1.3 check rules with target type of Constant value 2.2 check rules with source type of CardProfile 2.2.1 check rules with target type of ApplicatinProfile chekc target value in all
		 * ApplicationProfile compare source value with all target each other ApplicationProfile 2.2.2 regardless rules with target type of CardProfile/KeyProfile/LoadFileProfile 2.2.3 check rule of target of constant value
		 */
		String source;
		String rule;
		String target;
	...
	boolean ruleCheck(String source, String rule, String target) {
		...
		// value compare
		if (rule.equals(">")) {
			if (Integer.parseInt(source) > Integer.parseInt(target))
				return (true);
		}
		if (rule.equals("<")) {
			if (Integer.parseInt(source) < Integer.parseInt(target))
