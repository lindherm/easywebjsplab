...
/**
Declarations are used to specify the specific data requirements for a script. Any declaration��s Name attribute
must match up with an identically named DataElement defined at the ApplicationProfile.DataElement branch of
the XML document. It is possible to override certain attributes, with different values.
*/
public class apDeclaration extends ProfileNode
{
/**
Name of the data element. This is the name
referenced by the Scripts associated with an
Application Profile. Any declaration is only
valid if there exists a matching DataElement
in the ApplicationProfile (at the
ApplicationProfile.DataElement branch of the
document).
...
this data element that must be populated from
an external data source before the script is
executed. When this value is FALSE, the
data element could be a constant value or a
variable used by the script during script execution.
Example: true
*/
...
2 Base10 representation of the Number
3 true or false
...
specified in Encoding. This replaces any
definition in the matching DataElement for the
Value attribute.
Example: Fred
*/
public		String Value;
...
external data or parameters which need to be
updated during script execution for a
particular card holder. As opposed to the
Update flag, this updating of the data element
will be in real time and is expected to create a
data value for each cardholder. This replaces
any definition in the matching DataElement
for the ReadWrite attribute.
*/
...
used by subsequent script execution This
replaces any definition in the matching
DataElement for the Update attribute.
*/
public		String Update;
/**
Whether the data element is optional at all? If
yes, then this field will be set to true. If no,
then the field will be set to false. Useful for
specifying data which may or may not be
required by the script. This replaces any
definition in the matching DataElement for the
Required attribute.
*/
...
/**
MandatoryAudit
Whether or not, if this flag is provided, audit is
required for this data element.
True means if audit is not possible, the Script
should fail
False means if audit is not possible, the Script
doesn��t fail
