...
try{
	...
	if(nl.getLength()>0)Description=new apDescription(node);
	}catch(Exception e)
	{
//        e.printStackTrace();
	...
	try{
		...
		if(nl.getLength()>0)Revisions=new apRevisions(nl.item(0));
	}catch(Exception e)
	{
//        e.printStackTrace();
        log.error(e.getMessage());
	...
	if(nl.getLength()>0)ConflictRules=new apConflictRules(nl.item(0));
}catch(Exception e)
{
//    e.printStackTrace();
	...
	nl=xPathNode.getNodeList(xpString,node);
	if(nl.getLength()>0)ApplicationInfo=new apApplicationInfo(nl.item(0));
}catch(Exception e)
{
//    e.printStackTrace();
...
try{
	...
	if(nl.getLength()>0)
	{
		...
		{
			if(nl.getLength()>0)Key[i]=new apKey(nl.item(i));
		}
	}
}catch(Exception e)
{
...
/**
	add DataElement into ApplicationProifle,attribute of DataElement is same with existent DataElement(0)
	Name and  Length of DataElement is setted by user
	@param name Name of DataElement
	@param length Length of DataElement
 	@see apDataElement
*/
...
/**
	...
	@param name attribute of DataElement
	@see apDataElement
*/
...
/**
	Delete a Key from root of ApplicationProfile
...
putData��put data command��
0701 0702 0703
0D01
0E01 0E02
9102

putRSAKey��put key command��
8101 8103 8102 8104

putDESKey��put key command��
9103

changePin ��change/unblock pin command��
1101
...
/**
	...
	@parent kmc =true select ../profiles/hasKMC.js
				=false select ../Profile/noKMC.js
*/
...
/*	System.out.println(ap.ApplicationInfo.Version);
	ap.ApplicationInfo.setAttribute("Version","1.2.1");
	System.out.println(ap.ApplicationInfo.getAtrribute("Version"));
	System.out.println(ap.ApplicationInfo.getNodeValue());
	System.out.println(ap.ApplicationInfo.getNodeName());

	String data[]=new String[ap.DataElement.length];
	for(i=0;i<ap.DataElement.length;i++)
	{
		data[i]="Name="+ap.DataElement[i].Name+"; Ext="+ap.DataElement[i].External+"; Encoding="+ap.DataElement[i].Encoding;
...
/*	String des;
	...
	des=ap.Description.Description;
//  ap.setElement((Element)ap.document.getDocumentElement());
//	System.out.println(ap.getAtrribute("ProfileVersion"));
