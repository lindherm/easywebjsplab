 ...
 */
public class cpRevision extends ProfileNode {
	private Logger log = Logger.getLogger(cpRevision.class);
	/**
	 * UniqueID of the Profile that was utilized as a source for the current Profile.
	 */
	public String ProfileID;
	/**
	 * Version of Card Profile conforming to GP versioning specification. Version number is x.x.x where each x is a decimal number. If the version of the specification has no last x value, then version number is x.x.0. Example: 1.0.0
	 */
	 ...
	 * @see cpSignature
	 */
	public cpSignature Signature;

	public cpRevision(Node node) {
		super(node);
		Node attr;
		...
		try {
			...
			nl = xPathNode.getNodeList(xpString, node);
			if (nl.getLength() > 0)
				Signature = new cpSignature(nl.item(0));
