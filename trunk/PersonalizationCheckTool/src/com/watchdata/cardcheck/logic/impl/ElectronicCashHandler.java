...
public class ElectronicCashHandler extends BaseHandler {
	...
	/**
	 * @author peng.wang
	 * @param tradeMount 交易金额
	 * @param readerName 读卡器名称
	 * @param tradeLabel 交易界面显示详细控件
	 * @param termSupportUtil 判断终端性能
	 * @return
	 */
	public  boolean ECLoad(int tradeMount,String readerName,JLabel tradeLabel,TermSupportUtil termSupportUtil){
		...
		param.put("9F02",WDStringUtil.paddingHeadZero(
		...
		try{
			...
			if(WDAssert.isNotEmpty(result.get("88"))){
				...
				//GET DATA 报告
				/*apduSendANDRes.setSendAPDU(reportNoreqAndRes);
				apduSendANDRes.setDes("GET DATA");
				apduSendANDRes.setResponseAPDU(reportNoreqAndRes);
				apduSendANDRes.setTagDes(dataMap);
				genWordUtil.add(apduSendANDRes);*/
				...
				//动态数据认证报告
				/*apduSendANDRes.setSendAPDU(reportNoreqAndRes);
				apduSendANDRes.setDes("DDA");
				apduSendANDRes.setResponseAPDU(reportNoreqAndRes);
				apduSendANDRes.setTagDes(new HashMap<String,String>());
				genWordUtil.add(apduSendANDRes);*/
				/*genWordUtil.add("SignedDynmicData:" + signedDynmicData);
				genWordUtil.add("DdolDataList:" + ddolDataList);
				genWordUtil.add("IssuerPKCert:" + issuerPKCert);
				genWordUtil.add("IssuerPKReminder:" + issuerPKReminder);
				genWordUtil.add("IssuerPKExponent:" + issuerPKExponent);
				genWordUtil.add("IcPKCert:" + icPKCert);
				genWordUtil.add("IcPKExp:" + icPKExp);
				genWordUtil.add("IcPKReminder:" + icPKReminder);
				genWordUtil.add("CaPKIndex:" + caPKIndex);
				genWordUtil.add("StaticDataList:" + staticDataList);
				genWordUtil.add("Pan:" + pan);
				genWordUtil.add("Rid:" + rid);*/
				...
				//ARQC报告
				/*apduSendANDRes.setSendAPDU(reportNoreqAndRes);
				apduSendANDRes.setDes("Check ARQC");
				apduSendANDRes.setResponseAPDU(reportNoreqAndRes);
				apduSendANDRes.setTagDes(new HashMap<String,String>());
				genWordUtil.add(apduSendANDRes);*/
			...
			logger.error("ElectronicCashHandler ECLoad unknown error",e);
			return false;
		}finally{
			NDC.pop();
			NDC.remove();
		...
		try{
			...
			if(WDAssert.isNotEmpty(result.get("88"))){
				...
				if(Integer.parseInt(param.get("9F02")) > Integer.parseInt(balance)){
					...
					/*apduSendANDRes.setSendAPDU(reportNoreqAndRes);
					apduSendANDRes.setDes("OnLine Check ARQC");
					apduSendANDRes.setResponseAPDU(reportNoreqAndRes);
					apduSendANDRes.setTagDes(new HashMap<String,String>());
					genWordUtil.add(apduSendANDRes);*/
					genWordUtil.add("验证ARQC中使用的数据");
			...
			logger.error("ElectronicCashHandler ECPurcharse unknown error",e);
			return false;
		}finally{
			NDC.pop();
			NDC.remove();
