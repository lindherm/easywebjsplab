	...
	public StaticDataCalibrationResultsPanel() {
		...
		stopButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent arg0) {
				int y = JOptionPane
						.showConfirmDialog(null, "确定要停止当前检测吗！", pm
				...
				if (y == 0) {
					...
					/*
					 * try { check.join(); } catch (InterruptedException e) {
	...
	class CheckStaticDataThread implements Runnable {
		...
		public void run() {
			...
			if (reader == null || "".equals(reader.trim())) {
				JOptionPane.showMessageDialog(null, "请先配置读卡器!", "提示框",
			...
			if (!apduChannel.init(reader)) {// 连不上卡和读卡器区分
				JOptionPane.showMessageDialog(null, "连不上读卡器!", "提示框",
						JOptionPane.INFORMATION_MESSAGE);
				disable(true);
				return;
			}
			...
			// 从数据库中读取数据
			Map<String, Map<String, StaticData>> staticOriDataMap = staDataDetect
					.getCardDataFromDatabase();
			if (staticOriDataMap == null) {
				JOptionPane.showMessageDialog(null, "数据库不可用！", pm
						.getString("mv.testdata.InfoWindow"),
			...
			if (staticOriDataMap .size()==0) {
				...
				int y = JOptionPane
				.showConfirmDialog(null, "没有配置检测数据!继续将只读取卡片静态数据！", pm
						.getString("mv.testdata.InfoWindow"),
						JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE);
					if (y == 1) {
			...
			try {
				...
				if (allAppData == null || allAppData.size() == 0 ) {
					JOptionPane.showMessageDialog(null, "卡片应用数据为空！", pm
							.getString("mv.testdata.InfoWindow"),
							JOptionPane.ERROR_MESSAGE);
					 disable(true);
					return;
				...
				Map<String, Map<String, StaticData>> staticDataMap = staDataDetect
						.check(allAppData, staticOriDataMap);
				...
				while (it.hasNext()) {
					...
					while (tagIt.hasNext()) {// 对比每一个tag和value
						...
						Object[] data = new Object[] { stacData.getAppType(),
								stacData.getTag(), stacData.getOriValue(),
								stacData.getCardValue(), stacData.getDscrpt(),
								stacData.getIsMatch() };
				...
				JOptionPane.showMessageDialog(null, "检测出错!", "提示框",
						JOptionPane.ERROR_MESSAGE);
				progressBar.setVisible(false);
			} finally {
				status = 1;
