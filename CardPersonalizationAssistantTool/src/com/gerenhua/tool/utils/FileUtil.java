...
public class FileUtil {
	...
	public static void writeVector(Vector<Vector<String>> vector, File file) {
		try {
	...
	public static Vector<Vector<String>> getConfigFromFile() {
		...
		/*
		 * List<String> rulesList = readeFile(); if (rulesList.isEmpty()) { return data; } for (int i = 0; i < rulesList.size(); i++) { Vector<String> ruleList = new Vector<String>(); String[] temp = rulesList.get(i).split("[|]"); // System.out.println(temp.toString()); for (int j = 0; j < temp.length; j++) { String maskText = temp[j]; if (j == 0) { int idx = Integer.valueOf(temp[j]); switch (idx) { case 0: maskText = "固定值"; break;
		 * 
		 * case 1: maskText = "动态增长数"; break; case 2: maskText = "校检位"; break; case 3: maskText = "随机数";// "固定值", "动态增长数", "校检位", "随机数" break; default: break; } } // System.out.println(i+":"+maskText); // data[i][j - 1] = maskText; ruleList.add(maskText); } data.add(ruleList);
		 */
		// }
		return data;
	}
